...
func ListGroupKategori(c *gin.Context) {
	...
	sql := "  SELECT a.*,b.proses_uang FROM tbl_group_kategoris as a " +
		...
		" and a.kd_group not in(" + str_kd_group + ")  order by b.proses_uang "
...
func ListKategoriUang(c *gin.Context) {
	...
	var str_kd_kategori string
	rows, _ := db.Raw("SELECT kd_kategori FROM tbl_link_kategoris where link_name in('form_biaya_spp','form_biaya_ppdb') ").Rows()
	defer rows.Close()
	for rows.Next() {
		rows.Scan(&result_kd_kategori)
		if nomor == 0 {
			str_kd_kategori = result_kd_kategori
		} else {
			str_kd_kategori = str_kd_kategori + "," + result_kd_kategori
		}
		nomor++
	}
	...
	sql := " SELECT a.*,b.nm_group FROM tbl_kategori_uangs as a  " +
		...
		" where a.flag_aktif=0 and b.flag_aktif=0 " +
		" and a.kd_kategori not in (" + str_kd_kategori + ") order by b.nm_group "
